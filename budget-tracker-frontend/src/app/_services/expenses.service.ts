...
export class ExpensesService {
    ...
    getExpensesList(start: string, end: string) {
        ...
        return lastValueFrom(
            this._http.get<ExpensesForListDto>(`${this._baseUrl}`, {params})
        );
    }

    getExpense(expenseId: number) {
        return lastValueFrom(
            this._http.get<ExpenseDetailsDto>(`${this._baseUrl}/${expenseId}`)
        );
    }
    ...
    updateExpense(expenseId: number, updateExpense: UpdateExpenseDto) {
        return lastValueFrom(
            this._http.put(`${this._baseUrl}/${expenseId}`, updateExpense)
        );
    }

    deleteExpense(expenseId: number) {
        return lastValueFrom(
            this._http.delete(`${this._baseUrl}/${expenseId}`)
        );
    }
}
