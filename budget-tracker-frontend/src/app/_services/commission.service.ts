...
export class CommissionService {
    ...
    getCommissionList(start: string, end: string) {
        ...
        return lastValueFrom(
            this._http.get<CommissionForListDto>(`${this._baseUrl}`, {params})
        );
    }

    getCommission(commissionId: number) {
        return lastValueFrom(
            this._http.get<CommissionDetailsDto>(
                `${this._baseUrl}/${commissionId}`
            )
        );
    }
    ...
    createCommission(createCommmissionDto: CreateCommissionDto) {
        return lastValueFrom(
            this._http.post(this._baseUrl, createCommmissionDto)
        );
    }
    ...
    createCashOutCommission(createCashOutCommissionDto: CreateCommissionDto) {
        return lastValueFrom(
            this._http.post(
                `${this._baseUrl}/cashout`,
                createCashOutCommissionDto
            )
        );
    }
    ...
    updateCommission(
        commissionId: number,
        updateCommission: UpdateCommissionDto
    ) {
        return lastValueFrom(
            this._http.put(`${this._baseUrl}/${commissionId}`, updateCommission)
        );
    }

    deleteCommission(commissionId: number) {
        return lastValueFrom(
            this._http.delete(`${this._baseUrl}/${commissionId}`)
        );
    }
}
