...
export class FixedExpensesService {
    ...
    getFixedExpensesList(start: string, end: string) {
        ...
        return lastValueFrom(
            this._http.get<FixedExpensesForListDto>(`${this._baseUrl}`, {
                params,
            })
        );
    }
    ...
    getFixedExpense(fixedExpenseId: number) {
        return lastValueFrom(
            this._http.get<FixedExpenseDetailsDto>(
                `${this._baseUrl}/${fixedExpenseId}`
            )
        );
    }
    ...
    createFixedExpense(createFixedExpenseDto: CreateFixedExpenseDto) {
        return lastValueFrom(
            this._http.post(this._baseUrl, createFixedExpenseDto)
        );
    }

    updateFixedExpense(
        fixedExpenseId: number,
        updateFixedExpense: UpdateFixedExpenseDto
    ) {
        return lastValueFrom(
            this._http.put(
                `${this._baseUrl}/${fixedExpenseId}`,
    ...
    deleteFixedExpense(fixedExpenseId: number) {
        return lastValueFrom(
            this._http.delete(`${this._baseUrl}/${fixedExpenseId}`)
        );
    }
}
